...
def plot_confusion_matrix(y_true, y_pred, model_name="Model"):
    """
    Plots and saves a confusion matrix for classification results.
    """
    cm = confusion_matrix(y_true, y_pred)

    plt.figure(figsize=(6, 5))
    sns.heatmap(cm, annot=True, fmt='d', cmap='Blues',
                xticklabels=['Black Wins/Draw', 'White Wins'],
                yticklabels=['Black Wins/Draw', 'White Wins'])
    plt.xlabel('Predicted')
    plt.ylabel('Actual')
    plt.title(f'Confusion Matrix - {model_name}')

    filepath = f'results/confusion_matrix_{model_name.lower().replace(" ", "_")}.png'
